...
(function($) {
  ...
  $.fn.smoothChange = function(options) {
    ...
    //set defaults
    options = options || {};
    options.range = options.range || 5; //how many color steps
    options.scale = options.scale || 1; //how large of a change from each step
    options.speed = options.speed || 40; //how fast colors change
    options.red = options.red || 'on';  //accepts off for red to stay the same or a number to multiply red by
    options.green = options.green || 'on'; //accepts off for green to stay the same or a number to multiply green by
    options.blue = options.blue || 'on'; //accepts off for blue to stay the same or a number to multiply blue by
